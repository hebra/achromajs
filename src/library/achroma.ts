...
class AchromaJS {
  private minifiedCSS: string = `
    PLACEHOLDER_FILTER_CSS
    `;

  private svgFilters: string = `
    PLACEHOLDER_SVG_FILTERS
    `;
  ...
  public init() {
    const savedTabs =
      JSON.parse(localStorage.getItem("achromajsSelectedFilter") || "{}") || {};
    ...
    // Inject the SVG filters
    const svgId = "achromajs-svg-filters";
    if (
      this.svgFilters && this.svgFilters.includes("<svg") &&
    ...
    new FiltersUIList(this.popup).build((ev: Event) => this.filterClicked(ev), [
  ...
  applyFilter(selectedCSSClass: string, skipWorkaround: boolean = false) {
